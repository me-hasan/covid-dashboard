...
using(district) ORDER BY r.test_positivity DESC");
    ...
    public function getDhakaPositiveRateData()
    {
        $sql = "SELECT
                a.date_of_test,
                a.district,
    ...
    public function uploadData(Request $request)
    {
        ...
        if ($request->hasFile('data_file')) {
            ...
            Storage::putFileAs('public', $file, $request->data_file->getClientOriginalName());
    ...
    public function dailyInfectedChart(Request $request)
    {
        ...
        $five_dayMovingAvgInfected = DB::select("
                    SELECT a.date_of_test, SUM(a.total_positive) as total_positive, ROUND( ( SELECT AVG(b.total_positive)
                        FROM daily_data_iedcr AS b
                        WHERE DATEDIFF(a.date_of_test, b.date_of_test) BETWEEN 0 AND 4
                          ), 2 ) AS 'five_dayMovingAvgInfected'
                    FROM `daily_data_iedcr` as a
                    GROUP BY a.date_of_test");
