    ...
    <div class="row">
        <div class="col-xl-4 col-lg-4 col-md-12">
            <div class="card-header pb-0">
                <h5 class="header-title pb-1">গত ১৪ দিনে পরীক্ষা</h5>
            </div>
            <div class="card purple-card">
                <h1 class="font-weight-bold mb-1 b1 fs-40">{!! isset($last_14_days['getLast14DaysTestData'][0]) ? formatInBanglaStyle($last_14_days['getLast14DaysTestData'][0]->curr_fourtten_days_test) : ' ' !!}</h1>
                <span class="text-muted b1 fs-16"><i class="{{$class_1}}"></i> পূর্ববর্তী ১৪ দিনে পরীক্ষার চেয়ে {!! isset($last_14_days['getLast14DaysTestData'][0]) ? formatInBanglaStyle(floor($last_14_days['getLast14DaysTestData'][0]->Difference)) : ' ' !!} টি @if(isset($last_14_days['getLast14DaysTestData'][0]->Difference) && $last_14_days['getLast14DaysTestData'][0]->Difference < 1) কম  @else বেশি @endif </span>
            </div>
        </div>
        ...
        <div class="col-xl-4 col-lg-4 col-md-12">
            ...
            <div class="card-body pt-0">
                <h1 class="font-weight-bold mb-1 b1 fs-40">{!! isset($last_14_days['getLast14DaysinfectedData'][0]) ? formatInBanglaStyle($last_14_days['getLast14DaysinfectedData'][0]->curr_fourtten_days_infected_person) : ' ' !!} </h1>
                <span class="text-muted b1 fs-16"><i class="{{$class_2}}"></i> পূর্ববর্তী ১৪ দিনে আক্রান্তের চেয়ে {!! isset($last_14_days['getLast14DaysinfectedData'][0]) ? formatInBanglaStyle(floor($last_14_days['getLast14DaysinfectedData'][0]->Difference)) : ' ' !!} জন @if(isset($last_14_days['getLast14DaysinfectedData'][0]->Difference) && $last_14_days['getLast14DaysinfectedData'][0]->Difference < 1) কম  @else বেশি @endif </span>
            </div>
        </div>
                ...
                <div class="col-sm-4">
                    <div class="btn-group">
                        <div class="col-md-6 pl-0">
                            <input class="form-control national_in_trend_form_date" placeholder="From Date" type="date" name="from_date" value="{{ request()->get('from_date') }}">
                        </div>
                        <div class="col-md-6 pl-0">
                            <input class="form-control national_in_trend_to_date" placeholder="To Date" type="date" name="to_date" value="{{ request()->get('to_date') }}">
                        </div>
                        <div class="btn-group">
                            <button class="btn btn-primary-color pl-0 national_daily_in_trend_search" type="submit">
                    ...
                    <div class="col-xl-8 col-md-8 col-sm-8" style="float:right">
                        <div class="btn-group" style="float:right">
                            <div class="col-md-5 pl-0">
                                <input class="form-control national_in_trend_search_from_date" placeholder="From Date" type="date" name="from_date" value="{{ request()->get('from_date') }}">
                            </div>
                            <div class="col-md-5 pl-0">
                                <input class="form-control national_in_trend_search_to_date" placeholder="To Date" type="date" name="to_date" value="{{ request()->get('to_date') }}">
                            </div>
                            <div class="btn-group">
                                <button class="btn btn-primary-color pl-0 national_in_trend_search" type="submit">
                                    <svg class="header-icon search-icon" x="1008" y="1248" viewBox="0 0 24 24"
                                         height="100%" width="100%" preserveAspectRatio="xMidYMid meet"
                                         focusable="false">
                                        <path d="M0 0h24v24H0V0z" fill="none"/>
...
@push('custom_script')
    <script>
        ...
        $('.national_daily_in_trend_search').on('click', function (e){
            e.preventDefault();
        ...
        function national_daily_infected_chart(data) {
            ...
            Highcharts.chart('national_dialy_infected_trend', {
                ...
                series: [{
                    name: 'দৈনিক আক্রান্ত',
                    type: 'column',
                    data: JSON.parse(data.infected),

                }, {
                    name: 'দৈনিক আক্রান্ত (৫ দিনের  চলমান গড়)',
                    type: 'spline',
                    data: JSON.parse(data.avg),
                }]
            });
        }
        ...
        $('.national_in_trend_search').on('click', function (e){
            e.preventDefault();
            var url = new URL('{!! route('hpm.get_national_infected_trend') !!}');
            var to_date = $('.national_in_trend_search_to_date').val();
            var from_date = $('.national_in_trend_search_from_date').val();
            var national_infected_trendData= ajaxCallWithUrl(url, from_date,to_date);
            if(national_infected_trendData) {
                nationalInfectedTrend(national_infected_trendData);
            }


        });

        function nationalInfectedTrend(data) {
            Highcharts.chart('national_infected_trend', {
                chart: {
                    height: 460,
                    style: {
                ...
                series: [{
                    name: 'সংক্রামিত',
                    data: JSON.parse(data.row1_left_trend_infected_data),
                    type : 'area',
                    marker:{symbol:'circle'}
