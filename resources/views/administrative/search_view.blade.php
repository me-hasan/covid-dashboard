            ...
            <div class="btn-group mt-2 mb-2 mr-1">
                <select class="btn btn-outline-primary dropdown-toggle division" name="division" id="">
                    ...
                    @foreach($divisions as $division)
                        <option @if(request()->get('division') == $division->division) selected @endif value="{{$division->division}}"><?php echo en2bnTranslation($division->division); ?></option>
                    @endforeach
                </select>
            </div>
            <div class="btn-group mt-2 mb-2 mr-1">
        ...
        $('.district').change(function (e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.status == 1) {
                        $('.upazilla').empty();
                        html = '<option value="">উপজেলা</option>';
                        for (var i = 0; i < data.data.length; i++) {
